...
<HTML>
<HEAD>
		<TITLE>AgroDeal | Search results</TITLE>
...
<BODY>
		...
		<tr>
			...
			<form action="search_product.php" method="get">
				<input class="search-txt" type=text name="search" placeholder="Search Your Product" value="<?php echo $query; ?>">
		...
		<div class="wrapper">
			<div class="product-box">
				...
				<div class="product-grid">
					...
					<?php 
						$sql="SELECT image,prod_name,prod_price,prod_desc FROM products WHERE prod_name LIKE '%$query%' OR prod_desc LIKE '%$query%'";

						if ($result=mysqli_query($conn,$sql)) {
                        // Fetch one and one row
                        if(mysqli_num_rows($result)<1) {
                            echo '<div class="product"><h2>No Products Found.</h2></div>';
                        }
                        else {
